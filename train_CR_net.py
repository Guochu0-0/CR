...
parser.add_argument("--lr", default=1e-3, type=float, help="Learning rate, e.g. 0.01")
...
parser.add_argument("--use_sar", default=True, type=bool, help="whether to use SAR or not")
...
parser.add_argument("--root1", default='../../../Data/sen12ms-cr-ts/train', type=str, help="path to your copy of SEN12MS-CR-TS")
parser.add_argument("--root2", default='../../../Data/sen12ms-cr-ts/test', type=str, help="path to your copy of SEN12MS-CR-TS validation & test splits")
parser.add_argument("--root3", default='../../../Data/sen12ms-cr', type=str, help="path to your copy of SEN12MS-CR for pretraining")
...
dt_train    = SEN12MSCRTS(os.path.expanduser(opts.root1), split='train', region=opts.region, sample_type=opts.sample_type, sampler = 'random' if opts.vary_samples else 'fixed', n_input_samples=opts.input_t, import_data_path=import_from_path('train', opts), min_cov=opts.min_cov, max_cov=opts.max_cov)
dt_val      = SEN12MSCRTS(os.path.expanduser(opts.root2), split='val', region='all', sample_type=opts.sample_type , n_input_samples=opts.input_t, import_data_path=import_from_path('val', opts)) 
dt_test     = SEN12MSCRTS(os.path.expanduser(opts.root2), split='test', region='all', sample_type=opts.sample_type , n_input_samples=opts.input_t, import_data_path=import_from_path('test', opts))

f, g = torch.Generator(), torch.Generator()
f.manual_seed(opts.rdm_seed + 0)  # note:  this may get re-seeded each epoch
g.manual_seed(opts.rdm_seed)      #        keep this one fixed
